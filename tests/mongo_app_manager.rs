...
async fn mongo_resolves_by_id_and_key_and_filters_disabled() {
    ...
    coll.insert_many(vec![
        ...
        "max_backend_events_per_second": 500_i32,
    ...
    assert_eq!(
        ...
        "a document omitting the field is an absent override, not a zero one"
    );
...
#[tokio::test]
async fn mongo_negative_rate_override_fails_the_lookup() {
    let client = Client::with_uri_str(&uri())
        .await
        .expect("connect Mongo (is pylon-test-mongo up on 27018?)");
    let coll = client
    ...
    let n = uuid::Uuid::new_v4().to_string();
    let (id, key) = (format!("neg-{n}"), format!("negkey-{n}"));
    coll.insert_one(
        doc! { "id": &id, "key": &key, "secret": "sec", "name": "Neg", "capacity": 0_i32,
        "client_messages_enabled": false, "subscription_count_enabled": false,
        "enabled": true, "max_backend_events_per_second": -1_i32, "webhooks": [] },
    )
    .await
    .unwrap();
    ...
    let m = MongoAppManager::connect(&uri()).await.unwrap();
    assert!(
        m.by_id(&id).await.is_err(),
        "a negative limit must not resolve as an unlimited app"
    );
}
