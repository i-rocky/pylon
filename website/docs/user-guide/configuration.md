...
| `PYLON_LOG_FORMAT` | `text` | Log output format: `text` (human-readable, the default) or `json` (one JSON object per line: `timestamp`, `level`, `target`, and the event's structured fields under `fields`, with the log message at `fields.message`). Any other value is a startup error, and because this variable is parsed before any subscriber exists, that one failure is reported as plain text on stderr rather than in the selected format. Verbosity is still `RUST_LOG`. |
...
| `PYLON_CLUSTER_ENVELOPE_COMPAT` | `true` | Emit the Redis cluster relay envelope in its compat shape — every relayed frame travels with BOTH the legacy `event` field and the `frame_b64` field, so 0.2.x↔0.3.x mixed fleets relay in both directions during rolling upgrades. Set `0`/`false`/`off` — **only once every node runs a build that ships this knob** — to drop the legacy `event` field for frame-carrying envelopes and roughly halve cluster-bus bandwidth. **v0.3.0 does not qualify**: a 0.3.0 receiver cannot decode a compat-off envelope (its decoder still requires `event`) and drops it silently. Receivers on knob-shipping builds decode both shapes regardless of their own setting. See [Clustering: envelope compat](clustering.md#cluster-envelope-compat-post-030-fleets-only). |
...
| `PYLON_MAX_REST_REQUESTS_PER_SECOND` | `0` | Node-wide cap on REST API requests per second, applied **before** authentication so an unsigned flood costs no app-store lookup. Over the cap: `429` with `Retry-After`, `X-RateLimit-Limit` and `X-RateLimit-Remaining`. `/health`, `/ready`, `/metrics` and the admin API are never limited. Enforced per node with no cluster coordination, so N nodes behind a balancer jointly allow `N ×` this value. `0` = unlimited. |
| `PYLON_MAX_BACKEND_EVENTS_PER_SECOND` | `0` | Default per-app cap on published events per second (`POST /events` costs 1; `POST /batch_events` costs its event count). Overridden per app by `max_backend_events_per_second`. A non-zero value below `PYLON_MAX_BATCH_EVENTS` is refused at startup — a full-size batch could never be afforded — while a per-app override is not validated against the batch cap. Enforced per node with no cluster coordination, so N nodes behind a balancer jointly allow `N ×` this value. `0` = unlimited. |
| `PYLON_MAX_READ_REQUESTS_PER_SECOND` | `0` | Default per-app cap on REST read requests per second (`GET channels`, `GET channels/{name}`, `GET channels/{name}/users`). Overridden per app by `max_read_requests_per_second`. Enforced per node with no cluster coordination, so N nodes behind a balancer jointly allow `N ×` this value. `0` = unlimited. |
